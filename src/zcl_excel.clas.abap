  ...
  PUBLIC SECTION.
    ...
    METHODS output_to_excel
      IMPORTING
        ...
        !iv_sort_condition TYPE string OPTIONAL .

  PROTECTED SECTION.
  PRIVATE SECTION.
    METHODS get_excel
      EXPORTING eo_sheets           TYPE ole2_object
    ...
    METHODS   set_header
      IMPORTING io_ref_struc TYPE REF TO cl_abap_structdescr
      CHANGING  co_sheet     TYPE ole2_object.

    METHODS  generate_output_table
      IMPORTING it_table        TYPE ANY TABLE
                io_ref_struc    TYPE REF TO cl_abap_structdescr
      EXPORTING et_output_table TYPE tyt_excel_cell.

    METHODS   fill_sheet
      IMPORTING it_output_table TYPE tyt_excel_cell
                io_ref_struc    TYPE REF TO cl_abap_structdescr
                iv_sheet_name   TYPE string
      CHANGING  co_sheet        TYPE ole2_object.
    ...
    METHODS get_number_of_columns
      IMPORTING io_ref_struc                TYPE REF TO cl_abap_structdescr
      RETURNING VALUE(rv_number_of_columns) TYPE i.


    METHODS adjust_column_width
      IMPORTING iv_column_index TYPE i
                iv_width        TYPE i
      CHANGING  co_worksheet    TYPE ole2_object.
    ...
    METHODS return_subtable
      IMPORTING it_all_data  TYPE ANY TABLE
                it_key_field TYPE string
                it_key_value TYPE string
      EXPORTING et_subtable  TYPE ANY TABLE.
...
**********************************************************************
    ...
    IF iv_sort_condition IS INITIAL.
**********************************************************************
...
**********************************************************************
    ...
    ELSE.
      ...
      DATA: lt_table              TYPE REF TO data,
            ...
            lv_num_of_dsos        TYPE i,
            lv_cur_dso_nr         TYPE i,
      ...
      FIELD-SYMBOLS: <ft_table>           TYPE ANY TABLE,     " ultimately a table with one entry for each DSO
                     ...
                     <ft_dso_table>       TYPE ANY TABLE, " Table that contains all entries of one DSO
                     <fs_dso>             TYPE any,
                     <fs_cur_dso>         TYPE any,
      ...
      ASSIGN lt_table2->* TO <ft_dso_table>.
      ...
      SORT  <ft_table>  BY (iv_sort_condition).
      SORT  <ft_table2> BY (iv_sort_condition).

      " delete double entries with respect ot DSOS.
      DELETE ADJACENT DUPLICATES FROM  <ft_table> COMPARING (iv_sort_condition).

      " lv_num_of_DSOs now contains the number of DSOs
      DESCRIBE TABLE <ft_table> LINES lv_num_of_dsos.

**********************************************************************
* Generierung der Excel Instanz/Referenz
**********************************************************************
**********************************************************************
* Get an excel object with one sheet
**********************************************************************
...
**********************************************************************
* Generierung der Excel Instanz/Referenz - Ende
**********************************************************************

      lv_cur_dso_nr  = 0.

      LOOP AT <ft_table> ASSIGNING <fs_dso>.

        " Number of current DSO
        lv_cur_dso_nr  = lv_cur_dso_nr  + 1.

        ASSIGN COMPONENT iv_sort_condition OF STRUCTURE <fs_dso> TO <fs_cur_dso>.
        lv_sheet_name = <fs_cur_dso>.

        REFRESH <ft_dso_table>.

        return_subtable(
         EXPORTING
           it_all_data  = it_table
           it_key_field = iv_sort_condition
           it_key_value = lv_sheet_name
         IMPORTING
           et_subtable  = <ft_dso_table> ).
...
**********************************************************************
* Erzeugung des Header
**********************************************************************

        set_header(
          EXPORTING
            io_ref_struc = lr_struc
          CHANGING
            co_sheet     = lo_sheet ).

**********************************************************************
* Erzeugung des Paste contents.
**********************************************************************
        ...
        generate_output_table(
          EXPORTING
            io_ref_struc    = lr_struc
            it_table        = <ft_dso_table>
          IMPORTING
            et_output_table = lt_string ).
...
**********************************************************************
        ...
        lv_number_of_columns = get_number_of_columns(
           EXPORTING io_ref_struc = lr_struc ).

        DO lv_number_of_columns TIMES.

          autoformat_column_width(
            EXPORTING
              iv_column_index = sy-index
            CHANGING
              co_sheet        = lo_sheet ).
...
**********************************************************************
    LOOP AT it_table ASSIGNING FIELD-SYMBOL(<fs_table_line>).
      " fill the string with the contents of the fields
      LOOP AT io_ref_struc->components INTO DATA(ls_column).
        ASSIGN COMPONENT ls_column-name OF STRUCTURE <fs_table_line> TO FIELD-SYMBOL(<fs_field_content>).
        ...
        ELSE.
          IF <fs_field_content> IS ASSIGNED.
            CONCATENATE ls_string <fs_field_content> INTO ls_string SEPARATED BY cl_abap_char_utilities=>horizontal_tab.
          ENDIF.
        ENDIF.
      ENDLOOP.
